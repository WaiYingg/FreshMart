<?php
	...
	if (isset($_POST['submit'])) {
		...
		if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM login1 WHERE email='{$email}'")) > 0) {
			$msg = "<div class='alert alert-danger'>{$email} - This email address has already been used.</div>";
		} else {
			if ($password === $cpassword) {
				$sql = "INSERT INTO login1 (username, email, password, code, user_type) VALUES ('{$name}', '{$email}', '{$password}', '{$code}', '{$user_type}')";
